      ...
      >
        ...
        <div className="mt-2 md:mt-6 lg:mt-8 grid grid-cols-1 gap-1.5 md:gap-3 sm:grid-cols-2">
          {promptSuggestions.map((prompt, index) => (
            ...
            >
              <div className="flex-shrink-0 rounded-md bg-primary/10 p-1 md:p-2 text-primary">
                {prompt.icon}
              </div>
              <div className="flex items-center min-h-[28px] md:min-h-0">
                <p className="font-medium text-xs md:text-base text-card-foreground">{prompt.text}</p>
              </div>
            </motion.button>
          ))}
        </div>
