...
export const WelcomeScreen = ({ onPromptClick }: WelcomeScreenProps) => {
  return (
    // --- MODIFIED: Greatly reduced vertical padding for mobile (py-4) ---
    <div className="flex flex-col items-center justify-center text-center p-4 py-4 md:py-12">
      ...
      >
        {/* --- MODIFIED: Reduced logo size and margin for mobile --- */}
        <div className="mx-auto mb-3 h-10 w-10 md:h-16 md:w-16 flex items-center justify-center rounded-full bg-primary/10 text-primary">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 md:h-8 md:w-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22a10 10 0 0 0 10-10H2a10 10 0 0 0 10 10z"/><path d="m14.5 13-1.4-1.4a2 2 0 0 0-2.8 0L9 13"/><path d="M12 22V8"/><path d="m12 8-1.5-1.5a2.83 2.83 0 0 1 0-4 2.83 2.83 0 0 1 4 0 2.83 2.83 0 0 1 0 4L12 8z"/></svg>
        </div>
        
        {/* --- MODIFIED: Reduced text size for mobile --- */}
        <h1 className="text-2xl font-bold tracking-tight text-foreground sm:text-4xl">
          SAP Assistant
        </h1>
        {/* --- MODIFIED: Reduced margin-top for mobile --- */}
        <p className="mt-2 text-base text-muted-foreground sm:text-lg">
          {getGreeting()}! How can I help you today?
        </p>

        {/* --- MODIFIED: Reduced margin-top and gap for mobile --- */}
        <div className="mt-4 md:mt-8 grid grid-cols-1 gap-2 sm:gap-3 sm:grid-cols-2">
          {promptSuggestions.map((prompt, index) => (
            <motion.button
              ...
              className="group flex items-start gap-3 rounded-lg border bg-card p-3 text-left transition-colors hover:bg-muted/50 sm:gap-4 sm:p-4"
            >
              {/* --- MODIFIED: Reduced padding for mobile --- */}
